...
const SearchBar = (props: {className: string, category: string, changeCategory: React.Dispatch<React.SetStateAction<string>>, inputValue: string, setInput: React.Dispatch<React.SetStateAction<string>>}) => {
    const [ openSelect, setOpenSelect ] = useState(false)

    return (
        <Form className={`bg-slate-100 flex items-center rounded-md shadow-md w-full dark:bg-slate-800 dark:text-white p-0.5 px-1 ${props.className}`} id="searchbar" method='get' action={`/${props.category}`}>
